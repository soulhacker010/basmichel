...
export default function EditorFinished() {
  ...
  const { data: projects = [] } = useQuery({
    queryKey: ['finishedProjects'],
    queryFn: () => base44.entities.Project.filter({ status: 'klaar' }, '-completed_date'),
  });
  ...
  const filteredProjects = projects
    ...
    .sort((a, b) => {
      ...
      return new Date(b.shoot_date) - new Date(a.shoot_date);
    });

  return (
    <div className="max-w-7xl mx-auto">
      <div className="mb-8">
        <h1 className={cn("text-2xl font-light", darkMode ? "text-gray-100" : "text-gray-900")}>Finished Projects</h1>
        <p className={cn("mt-1", darkMode ? "text-gray-400" : "text-gray-500")}>Completed projects</p>
      </div>
