...
export default function EditorInProgress() {
  ...
  const { data: projects = [] } = useQuery({
    queryKey: ['inProgressProjects'],
    queryFn: () => base44.entities.Project.filter({ status: 'wordt_bewerkt' }, '-updated_date'),
  });
  ...
  return (
    <div className="max-w-7xl mx-auto">
      <div className="mb-8">
        <h1 className={cn("text-2xl font-light", darkMode ? "text-gray-100" : "text-gray-900")}>In Progress</h1>
        <p className={cn("mt-1", darkMode ? "text-gray-400" : "text-gray-500")}>Projects currently being edited</p>
      </div>
            ...
            >
              ...
              <div className="p-6">
                ...
                <div className={cn("text-sm", darkMode ? "text-gray-500" : "text-gray-400")}>
                  {project.updated_date && format(new Date(project.updated_date), 'MMM d, yyyy')}
                </div>
              </div>
            </Link>
          );
